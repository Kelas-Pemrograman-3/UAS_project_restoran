    ...
    >
      ...
      <template v-slot:body="props">
          <q-tr :props="props">
            ...
            <q-td key="bayar" :props="props">{{ props.row.bayar }}</q-td>
   ...
   <q-dialog v-model="openedit" v-if="openedit">
      <q-card style="width: 700px; max-width:80vw;">
        ...
        <q-separator/>
          ...
          <q-input label ="total" v-model ="activedata.total"></q-input>
          <q-input label ="bayar" v-model ="activedata.bayar"></q-input>
          </q-card-selection>
...
export default {
  ...
  data () {
    return {
      ...
      columns: [
        ...
        { name: 'namaminuman', align: 'center', label: 'Nama Minuman', field: 'namaminuman', sortable: true },
        { name: 'jumlah', label: 'Jumlah', field: 'jumlah', sortable: true },
        { name: 'total', label: 'Total Harga', field: 'total', sortable: true },
        { name: 'bayar', label: 'Bayar', field: 'bayar', sortable: true },
  ...
  methods: {
    ...
    edit () {
      this.$axios.put('transaksi/updatetransaksi/' + this.activedata._id, this.activedata)
        .then(res => {
          if (res.data.error) {
    ...
    confirm (id) {
      this.$q.dialog({
        ...
        this.$axios.delete('transaksi/deletetransaksi/' + id)
          .then(res => {
            if (res.data.error) {
              this.showNotif(res.data.pesan, 'negative')
            } else {
