   ...
   <q-card>
    ...
    >
      ...
      <template v-slot:body="props">
          <q-tr :props="props">
            <q-td key="kodemakanan" :props="props">{{ props.row.kodemakanan }}</q-td>
            <q-td key="namamakanan" :props="props">{{ props.row.namamakanan }}</q-td>
            <q-td key="hargamakanan" :props="props">{{ props.row.hargamakanan }}</q-td>
   ...
   <q-dialog v-model="openedit" v-if="openedit">
      <q-card style="width: 700px; max-width:80vw;">
        ...
        <q-separator/>
          ...
          <q-input label ="kodemakanan" v-model ="activedata.kodemakanan"></q-input>
          <q-input label ="namamakanan" v-model ="activedata.namamakanan"></q-input>
          <q-input label ="hargamakanan" v-model ="activedata.hargamakanan"></q-input>
          </q-card-selection>
...
export default {
  ...
  data () {
    return {
      ...
      columns: [
        { name: 'kodemakanan', align: 'center', label: 'kodemakanan', field: 'kodemakanan', sortable: true },
        { name: 'namamakanan', align: 'center', label: 'Makanan', field: 'namamakanan', sortable: true },
        { name: 'hargamakanan', label: 'hargamakanan', field: 'hargamakanan', sortable: true },
  ...
  methods: {
    ...
    edit () {
      this.$axios.put('makanan/updatemakanan/' + this.activedata._id, this.activedata)
        .then(res => {
          if (res.data.error) {
    ...
    confirm (id) {
      this.$q.dialog({
        ...
        this.$axios.delete('makanan/deletemakanan/' + id)
          .then(res => {
            if (res.data.error) {
              this.showNotif(res.data.pesan, 'negative')
            } else {
